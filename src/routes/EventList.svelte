<script lang="ts">
    ...
    // Add empty events to weeks without event at the right position in the events array
...
<div class="flex flex-row lg:flex-col gap-4 lg:gap-4 overflow-x-auto p-6 bg-white border-solid border rounded-xl border-gray-300 shadow-lg">
    {#each events as event}
        <EventListItem {event} {term} firstInWeek={event.id == "" || firstEventInWeek(events, event)} emptyWeek={event.id == ""} />
    {/each}
</div>
